...
//variables para la muneca x, y y.
var muneca_x = 0;
var muneca_y =0;
//comfirmasion del codigo para iniciar
var conf = false;
function startGame()
{
  document.getElementById("sta").innerHTML = "restart";
  if (conf == true)
  {
    playerscore = 0;
    pcscore = 0;
...
function preload()
{
  pong = loadSound("ball_touch_paddel.wav");
  gol = loadSound("missed.wav");
}

function setup(){
  
  var canvas =  createCanvas(700,600);
 canvas.parent("ping-pong");
 video = createCapture(VIDEO);
 video.size(700,600);
 video.parent("cam");
 pos_net = ml5.poseNet(video, modelLoaded);
 pos_net.on("pose", gotPoses);
}
function modelLoaded()
{
console.log("modelo cargado");
document.getElementById("status").innerHTML = "empesado";
}
  ...
  function gotPoses(results)
  {
    if (results.length > 0)
    {
      //rightWrist
      console.log(results);
      muneca_x = results[0].pose.rightWrist.x;

      muneca_y = results[0].pose.rightWrist.y;
      console.log("posision de las muneca derecha: x=" + muneca_x + " | y=" + muneca_y);
    }
  }
...
if (conf == true)
{
  ...
  //image(video ,0 ,0 ,700 ,600);
   ...
   rect(0,0,20,700);
     ...
     fill("blue");
     ellipse(muneca_x, muneca_y,20,20);
...
if(pcscore ==4){
    ...
    text("¡presiona el boton reinisio!",width/2,height/2+30);
    noLoop();
    pcscore = 0;
}
